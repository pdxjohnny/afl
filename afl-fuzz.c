  ...
  if (N_rp->ai_socktype == SOCK_STREAM) {
    ...
    if (client_fd == -1) {
      if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
        return -1; /* return to calling program, which will delay before
        retrying */
    ...
    * the returned client_fd) */
    ...
    if (o >= 0) {
      ...
      if (fcntl(client_fd, F_SETFL, o) < 0) {
        PFATAL("failed to reset non-blocking flag on client file descriptor "
               "(TCP)");
      }
    }
    while ((currreadlen =
    ...
    */
    ...
    {
      int received_one = 0;
      while ((currreadlen = recvfrom(
                  N_fd, recvbuf, MAXRECVBUFSIZE, MSG_DONTWAIT,
      ...
      * Local variables: */
      ...
      if (N_myaddr_valid == 0) {
        ...
        * in M_myaddr for subsequent calls. */
        ...
        if (getsockname(N_fd, (struct sockaddr *)(&N_myaddr), &N_myaddrlen) ==
            -1) {
          PFATAL(
        ...
        if (bind(N_fd, (struct sockaddr *)(&N_myaddr), N_myaddrlen) == -1) {
          PFATAL("Attempt to bind socket to source address & port failed (TCP "
                 "case)");
        }
        if (connect(N_fd, N_rp->ai_addr, N_rp->ai_addrlen) != -1) {
        } else {
...

      {
        ...
        * target process */
        ...
        /* seek to the beginning of the file */
        lseek(fd, 0, SEEK_SET);
        /* use sendfile() to transfer the data if possible because it is
        * efficient */
    ...
    if (N_timeout_given) {
      ...
      * up to three times (hard-coded) */
      ...
      /* ignore errors & accept possibility that delay can be shorter */
      {
        u32 N_tries = 3;
        nanosleep(&N_it, NULL);
        /* attempt to send up to 3 times (because of target process startup
        * time) */
        while (N_tries-- &&
...
static void write_stats_file(double bitmap_cvg, double eps) {
  ...
  fprintf(f, "start_time     : %llu\n"
          ...
          bitmap_cvg, unique_crashes, unique_hangs, last_path_time / 1000,
          last_crash_time / 1000, last_hang_time / 1000, exec_tmout, use_banner,
          orig_cmdline);
  /* ignore errors */
...
#define SP20 SP10 SP10
  ...
  SAYF(bV bSTOP "   bit flips : " cNOR "%-37s " bSTG bV bSTOP
                "    levels : " cNOR "%-10s " bSTG bV "\n",
       tmp, DI(max_depth));
  ...
  SAYF(bV bSTOP "  byte flips : " cNOR "%-37s " bSTG bV bSTOP
                "   pending : " cNOR "%-10s " bSTG bV "\n",
  ...
  SAYF(bV bSTOP " arithmetics : " cNOR "%-37s " bSTG bV bSTOP
                "  pend fav : " cNOR "%-10s " bSTG bV "\n",
  ...
  SAYF(bV bSTOP "  known ints : " cNOR "%-37s " bSTG bV bSTOP
                " own finds : " cNOR "%-10s " bSTG bV "\n",
  ...
  if (!blocks_eff_total) {
    ...
    sprintf(tmp2, "%0.02f%%", ((double)(blocks_eff_total - blocks_eff_select)) *
                                  100 / blocks_eff_total);
...
static void show_init_stats(void) {
  ...
  OKF("Here are some useful stats:\n\n"
      ...
      cGRA "    Test case count : " cNOR
      "%u favored, %u variable, %u total\n" cGRA "       Bitmap range : " cNOR
      "%u to %u bits (average: %0.02f bits)\n" cGRA
      "        Exec timing : " cNOR "%s to %s us (average: %s us)\n",
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "                  or a client of afl-fuzz (using the -L option).  If "
      "the\n"
  ...
  *BSD, so we can just let it slide for now. */
  ...
  if (read(fd, &fchar, 1) == 1 && fchar == '|') {
    ...
    SAYF(
        ...
        "    To avoid having crashes misinterpreted as hangs, please log in as "
        "root\n"
      ...
      if (!N_found1) {
        ...
        *N_found1 = 0;
        ...
        if (strcmp(N_type, "tcp") == 0) {
          ...
          N_hints.ai_flags =
              (AI_PASSIVE /* | AI_NUMERICSERV */); // COMMENTED OUT
          N_hints.ai_family = AF_UNSPEC;
